...
Envelope serializer, fountain transport, QR codec, CameraX analyzer, reconstruction, state machine, local diagnostics export, dan UX hardening sudah terintegrasi sampai milestone #22. Setelah uji nyata menunjukkan 0 accepted frame, live QR decoder diperbaiki untuk memakai detector pada full camera preview dan diagnostics sekarang memisahkan `qrNotFoundFrames`, `invalidProtocolFrames`, serta `transferIdMismatchFrames`. Diagnostics tetap hanya mencatat timing, frame counters, block counters, equation count, byte counters, dan goodput; payload file maupun isi QR tidak ditulis ke log atau export. Physical-device benchmark dan re-test tetap menjadi pekerjaan issue #23.
