...
CameraX `ImageAnalysis` memberikan frame CPU-accessible melalui `ImageAnalysis.Analyzer`; analyzer harus cepat dan memanggil `ImageProxy.close()` setelah selesai.[2] Gunakan `STRATEGY_KEEP_ONLY_LATEST` agar frame lama yang tidak lagi berguna dibuang ketika decoder tertinggal. Decode tidak boleh menjalankan operasi blocking panjang di main thread. Lifecycle receiver harus menjaga camera bind idempotent; event `ON_START` dan initial-state check tidak boleh membuat dua bind asynchronous untuk satu composable.

Analyzer harus memisahkan tiga langkah: ekstraksi frame kamera, QR decode, dan pengiriman hasil ke domain. `ImageProxy` harus ditutup dalam `finally`. Hasil decode harus disalin sebelum analyzer mengembalikan buffer ke CameraX. Jalur produksi memakai plane Y dari `YUV_420_888` sebagai luminance langsung; RGBA decoder tetap diuji sebagai compatibility path. Live QR decoder wajib menggunakan ZXing detector normal pada full preview; `PURE_BARCODE` hanya boleh menjadi fallback untuk bitmap yang telah tercrop. Regression fixture harus menempatkan QR di dalam frame yang lebih besar, menguji payload padat, dan mencakup fixture RGBA serta Y-plane.
...
Diagnostics diimplementasikan sebagai model immutable dan reducer pure. Snapshot mencatat `startedAtMs`, `endedAtMs`, `emittedFrames`, `acceptedFrames`, `duplicateFrames`, `rejectedFrames`, kategori rejection, `emittedBytes`, `acceptedBytes`, `systematicFrames`, `repairFrames`, `sourceBlocks`, `recoveredBlocks`, `equationCount`, `lastSequence`, `terminalPhase`, dan error code. Untuk physical-device diagnosis, snapshot juga mencatat jumlah frame camera yang dianalisis, resolusi, row/pixel stride, rotasi, ukuran buffer, luminance min/max/mean, dan `lastQrModules`. `elapsedMs` dan `goodputBytesPerSecond` dihitung dari snapshot, kemudian snapshot dapat diekspor sebagai JSON melalui `ACTION_CREATE_DOCUMENT`. Tidak ada nilai diagnostik yang boleh berisi payload, isi file, gambar camera, QR bytes, atau secret.
...
| #17 | Selesai pada checkpoint receiver: CameraX Preview/ImageAnalysis `1.4.1`, KEEP_ONLY_LATEST, YUV_420_888 Y-plane luminance analyzer, ImageProxy cleanup, permission states, QR decoder adapter, dan invalid-frame rejection. RGBA decoder tetap tersedia sebagai compatibility path dan fixture test. Physical-device QA tetap terbuka. |
...
| #23 | “Prepare a physical-device QA checklist and release candidate. Test with Wi-Fi, Bluetooth, and mobile data disabled after installation.” Diagnosis tiga rangkaian uji perangkat dicatat di [`docs/OPTICAL-TRANSFER-DEVICE-DIAGNOSIS.md`](./OPTICAL-TRANSFER-DEVICE-DIAGNOSIS.md); re-test berikutnya memakai telemetry camera dan QR render untuk memisahkan masalah input camera dari decoder. |
